...
> 二分的本质：在区间中找到满足某种性质的**边界**，使得区间二分为“不重叠”的两部分，每次选择答案所在的区间进行下一步处理，当区间长度为 1 时，即得到查找的答案（或可判断为无解）
...
直接使用 `mid := l + r >> 1` 进行中点计算存在整数溢出风险（如 Go 中可能超出 int 类型取值范围）

**正确计算方式**：`mid := l + (r - l) >> 1` 或者直接 `(l + r) / 2`
